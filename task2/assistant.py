...
class SmartAssistant:
    def __init__(self):
        ...
        self.context = {}
        self.conversation = []
        self.conversation_sqls = []  # 存储每轮的SQL语句
    ...
    def ask(self, question, question_id=None, seq_no=1):
        ...
        try:
            ...
            # 格式化输出
            content, images = self._format_output(df, parsed, question_id, seq_no)
    ...
    def _format_output(self, df, parsed, question_id, seq_no):
        """格式化输出结果"""
        try:
            if len(df) == 1:
                # 单一数值
                ...
                # 时间序列数据
                # 生成图表
                img_path = self.visualizer.generate_chart(df, question_id, seq_no)
                ...
                if len(values) > 1:
                    ...
                    content += f"最新值为{values.iloc[-1]:.2f}万元。"
                    ...
                    content = f"{company}的{metric}数据：{values.to_list()}。"
    ...
    def start_conversation(self, question_list, question_id):
        ...
        self.context = {}
        self.conversation = []
        self.conversation_sqls = []  # 重置SQL列表
        ...
        for idx, q_item in enumerate(question_list, 1):
            ...
            self.conversation_sqls.append(answer.get("sql", ""))  # 保存SQL
